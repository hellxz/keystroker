...
export default {
  ...
  methods: {
    ...
    stroking (event) {
      ...
      this.keyInputSum += 1 // 这里稍候记录
      console.log('正在输入中')
      // TODO 如果是删除键输入，现在输入的字坐标到退格键输入的坐标区间中，修改所有class为type-n
      // 如果是退格键
      //    记录输入前的坐标为获取当前输入字的坐标+1 的class值为type-n
      if (event.keyCode === 8) {
        // let closer = this.inputText.length
        // TODO
    ...
    checkTrue () {
      ...
      for (let i = 0; i < this.articleArr.length; i++) {
        ...
        if (this.articleArr[i] !== inputArr[i]) {
          ...
          $($('#article-block').children()[i]).html('<span class="type-err">' + this.articleArr[i] + '</span>')
        } else {
          $($('#article-block').children()[i]).html('<span class="type-suc">' + this.articleArr[i] + '</span>')
        }
      }
    },
    restroke () {
