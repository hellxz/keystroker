        ...
        <Split v-model="split2" mode="vertical">
            <div slot="top" class="split-pane" >
                <div id="control-area"  readonly>
                  <!--当还没开始打字的时候，展示载文-->
                  <span v-if="isStroking == false ">{{ article }}</span>
            ...
            <div slot="bottom" class="split-pane">
                <textarea id="follow-stroke-area" @keyup="startStroke">你工作在</textarea>
            </div>
        </Split>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      strokedSum: 0, // 已打字数
      defaultSplitSegmentSum: 3, // 默认文章字数分段数量
  ...
  methods: {
    ...
    startStroke () {
      ...
      if (this.currentSegment === 0) {
        ...
        let spansContact = ''
        let currentTypeArr = this.articleArr.splice(0, this.currentSegment * this.defaultSplitSegmentSum)
